...
{    
    ...
    function sum_data($field='' ,$table ,$where='')
    {
        ...
        $sum_data = mysqli_query($this->conn,"SELECT SUM($field) AS sum_rating from `$table` $where" ) or die(mysqli_error($this->conn));
        return $sum_data;
          ...
          function count($field,$table, $where='')
    {
        ...
        $select = mysqli_query($this->conn,"SELECT COUNT($field) AS total FROM $table $where") or die(mysqli_error($this->conn));
        return $select;
    }

    function delete_history($table, $where='', $other='')
    {
        if($where != '')
        {
        $where= 'where ' .$where;
        }
        return mysqli_query($this->conn,"delete FROM $table $where $other LIMIT 1");
    }

    function select_field($field='',$table ,$where='',$other='')
    {
        if($where != '')
        {
            $where= 'where ' .$where;
        }
        $select_field = mysqli_query($this->conn,"SELECT $field FROM $table $where $other") or die(mysqli_error($this->conn));
        return $select_field;
    }
